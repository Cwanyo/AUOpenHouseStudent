...
export class ViewEventPage {
  ...
  public checkAttend: boolean = false;
  ...
  ngOnDestroy(){
    console.log("ngOnDestory view-event");
  ...
  checkEventAttend(){
    this.restApiProvider.checkMyEventAttend(Number(this.event.TID))
    .then(result => {
      if((result as any).length == 0){
        //not in event
        this.checkAttend = false;
      }else{
        //already in event
        this.checkAttend = true;
      }
    })
    .catch(error =>{
  ...
  leaveEvent(){
    console.log("leaveEvent",this.event.TID);
    let confirm = this.alertCtrl.create({
      title: "Alert!",
      message: "Are you sure that you want to leave this event time?",
      enableBackdropDismiss: false,
      buttons: [{
        text: "Disagree"
      },{
        ...
        handler: () => {
          ...
          console.log('Agree clicked');
          this.presentLoading();
          this.restApiProvider.leaveEvent(Number(this.event.TID))
          .then(result => {
            console.log("leave event time success");
            this.checkAttend = false;
            this.loader.dismiss();
            var jsonData: any = result;
            if(jsonData.isSuccess){
          ...
          .catch(error =>{
            this.loader.dismiss();
            console.log("ERROR API : leaveEvent",error);
            if(error.status == 0){
              //show error message
              this.presentAlert("Cannot connect to server");
            }else{
